...
std::string HTTPServer::processHTTPCommand(const char *httpReqHeader)
{
    ...
    // we only accept GET
    if (cmd.length()<4 || (cmd.compare(0,4,"GET ") && cmd.compare(0,5,"POST ")))
    {
    ...
    if (!cmd.compare(0,4,"GET "))
    {
        ...
        reply = std::string("HTTP/1.1 200 OK\r\n"
    ...
    if (!cmd.compare(0,5,"POST "))
    {
        // parse URI and get corresponding content
        pos = cmd.find(" ",5);
        std::string uri(cmd,5,pos-4);

        std::string content = postContentFor(uri.c_str());
...
std::string HTTPServer::postContentFor(const char *uri)
{
    // try to find in cache
    StringMap::iterator it = htdocs.find(uri);
    if (it!=htdocs.end())
